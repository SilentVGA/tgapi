...
一个单文件、无 Web 框架、专为低内存 NAT 机器设计的 Telegram 验证码接收 Python 脚本。支持直接上传、下载 Telethon `.session` 文件。
...
> 注意：如果是受限的 Debian/Ubuntu 系统，请使用：
>
> ```bash
...
> 删除账号只会删除服务器上的本地 `.session`、`.session-journal` 和 `.meta.json` 文件，不会主动注销 Telegram 账号。
